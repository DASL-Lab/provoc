...
- [x] 0.6: Refactor and add methods
    ...
    - [ ] Consider a custom data data class that contains the data and the lineages (like `fuse()` but more powerful, but also hidden unless desired like `stats::model.matrix()`). This allows for more methods, such as checking lineages based on actual data and plotting lineages easily, and can help make the "`~ .`" notation even easier for the `provoc()` function. 
    - [ ] Add a vignette for a basic analysis and for a detailed analysis (using as many user-facing functions as is reasonable).
    - [ ] Examples for all user-facing functions
    - [ ] Optimizations: Stop making copies of the data!
...
- [ ] 0.8: Multiple methods
    - [ ] Basic linear models, GLMs, and LASSO for comparison
    ...
    - [ ] Temporally Varying Coefficient (Functions-on-Scalars) models when `by` is a datetime object.
