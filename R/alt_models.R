...
alcov <- function(coco, varmat,
    method = c("AlCoV-LM", "AlCoV-Robust", "AlCoV-NNLS", "AlCoV-Binom")) {
    ...
    bind_rows(lapply(method, function(methi) {
        ...
        if (methi == "AlCoV-LM") {
            ...
            res <- tryCatch(summary(MASS::rlm(freq ~ 0 + .,
                        data = df))$coef[, 1:2, drop = FALSE],
        ...
        res <- data.frame(variant = as.character(rownames(res)),
            ...
            time = round(as.numeric(difftime(Sys.time(), t0,
                        units = "secs")) / nrow(res), 4))
        row.names(res) <- NULL
        res
    }))
}
...
optim_methods <- function(coco, varmat,
    ...
    bind_rows(lapply(method, function(meth) {
        ...
        data.frame(variant = as.character(rownames(varmat)),
            ...
            time = round(as.numeric(difftime(Sys.time(), t0,
                        units = "secs")) / length(res$par), 5))
    }))
}

avg_freq <- function(fused,
    method = c("Simple Avg", "Simple Med",
        "Binomial_GLM", "Binomial_GLM_Quasi")) {
        ...
        if (method[1] %in% c("Simple Avg", "Simple Med")) {
            ...
            if (length(successes) > 3) {
                res <- glm(cbind(successes, failures) ~ 1,
                    family = ifelse(method[1] == "Binomial_GLM",
